...
// Get log ID from global (passed from index.php)
$id = new Id($logId ?? '');
$log = new Log($id);
...
if (!$log->exists()) {
    ...
    if (count($problems) > 0) {
        $problemString = count($problems) === 1 ? "problem" : "problems";
    ...
    <div class="row-inner<?= $shouldWrapLogLines ? "" : " no-wrap" ?>">
                        ...
                        <div class="information-list">
                            <?php foreach ($information as $info): ?>
                                <div class="information">
                                    <div class="information-label"><?= $info->getLabel(); ?>:</div>
                                    <div class="information-value"><?= $info->getValue(); ?></div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
                            ...
                            <?php foreach ($problems as $problem): ?>
                                <div class="problem">
                                    <div class="problem-header">
                                        ...
                                        <a href="/<?= $id->get() . "#L" . $number; ?>" class="btn btn-blue btn-no-margin btn-small">
                                            <span class="hide-mobile"><i class="fa fa-arrow-right"></i> Line </span>#<?= $number; ?>
                                        </a>
                                    </div>
                                    <div class="problem-body">
                                        <div class="problem-solution-headline">Solutions</div>
        ...
        <?php else: ?>
            <div class="not-found">
                ...
                <div class="not-found-text">The log you tried to open does not exist (anymore).<br />We automatically delete all logs not viewed in the last 90 days.</div>
